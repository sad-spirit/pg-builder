...
/**
 * "ON EMPTY" / "ON ERROR" behaviours as used by json_value() and regular column definition in json_table()
 *
 * Corresponds to json_value_on_behavior_clause_opt grammar production
 *
 * @property string|ScalarExpression|null $onEmpty
 * @property string|ScalarExpression|null $onError
 */
trait JsonValueBehaviours
{
    use HasBehaviours;
    ...
    {
        $this->setBehaviour($this->p_onEmpty, 'ON EMPTY', JsonKeywords::BEHAVIOURS_VALUE, $onEmpty);
    }
    ...
    {
        $this->setBehaviour($this->p_onError, 'ON ERROR', JsonKeywords::BEHAVIOURS_VALUE, $onError);
    }
}
